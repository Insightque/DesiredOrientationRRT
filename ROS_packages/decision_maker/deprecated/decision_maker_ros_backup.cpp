...
void plan_init(og::SimpleSetup* ss,double* start, double* goal, bool isLocal = false)
{
    ...
    if( isLocal )
    {
       ...
       // ss->setPlanner(ob::PlannerPtr(new og::CForest(ss->getSpaceInformation(),randomCheck,magneticfield_local, DesiredOrientation_local, true, false, true, "CFDO*(TFT_)")));
        ss->setPlanner(std::make_shared<ompl::geometric::RRTstar>(ss->getSpaceInformation(),randomCheck,magneticfield_local, DesiredOrientation_local,true,false,true,"DORRT_local"));
    }
    else
    {
 //       ss->setPlanner(ob::PlannerPtr(new og::CForest(ss->getSpaceInformation(),randomCheck,magneticfield, DesiredOrientation, true, false, true, "CFDO*(TFT)")));
      ...
      if( MODEDORRTSTAR != 0 )
      {
        ...
        ss->setPlanner(std::make_shared<ompl::geometric::RRTstar>(ss->getSpaceInformation(),randomCheck,magneticfield, DesiredOrientation,true,false,true,"DORRT_global"));
      }
      else
      {
          cout <<"RRT MODE"<<endl;
