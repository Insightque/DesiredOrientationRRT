...
void plan_init(og::SimpleSetup* ss,double* start, double* goal, bool isLocal = false)
{
    ...
    if( isLocal )
    {
//        ss->setPlanner(ob::PlannerPtr(new og::CForest(ss->getSpaceInformation())));   //orig jw
...
//         ss->setPlanner(ob::PlannerPtr(new og::CForest(ss->getSpaceInformation(),randomCheck,magneticfield_local, DesiredOrientation_local, true, false, true, "CFDO*(TFT_)")));
        ...
        ss->setPlanner(std::make_shared<ompl::geometric::RRTstar>(ss->getSpaceInformation(),randomCheck,magneticfield_local, DesiredOrientation_local,true,false,true,"DORRT_local"));//DORRT JW 1809
    }
    else
    {
